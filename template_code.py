...
img = Image.open("C:\\Users\\andre\\OneDrive\\Documents\\CODE_BD\\back1.png")
...
def button1_action():
    ...
    subprocess.run(["python3", "script1.py"])  # Înlocuiește cu calea către scriptul tău
    root.quit()  # Închide fereastra principală

def button2_action():
    # Redirecționează către scriptul2.py
    subprocess.run(["python3", "script2.py"])  # Înlocuiește cu calea către scriptul tău
    root.quit()  # Închide fereastra principală
...
def create_custom_button(text, command, y_position):
    ...
    frame.place(relx=0.5, rely=y_position, anchor="center", width=200, height=50)
    
    # Fundal gradient pentru buton
    canvas = tk.Canvas(frame, width=200, height=50, bd=0, highlightthickness=0)
    canvas.pack(fill="both", expand=True)

    # Setează culoarea de fundal inițială la albastru închis
    gradient = canvas.create_rectangle(0, 0, 200, 50, fill="#003366", outline="")
...
create_custom_button("Buton 1", button1_action, 0.4)
create_custom_button("Buton 2", button2_action, 0.6)
